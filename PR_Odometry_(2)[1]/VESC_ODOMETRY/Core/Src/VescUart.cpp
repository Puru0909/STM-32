...
void VescUart::setSerialPort(UART_HandleTypeDef* port)
{
	serialPort = port;
}
...
int VescUart::receiveUartMessage(uint8_t * payloadReceived) {
	...
	while ( HAL_GetTick() < timeout && messageRead == false) {
			...
			HAL_UART_Receive(serialPort, &data, sizeof(data), 10);
			messageReceived[counter++] = data;
...
int VescUart::packSendPayload (uint8_t * payload, int lenPay) {
	...
	// Sending package
	HAL_UART_Transmit(serialPort, messageSend, count, _TIMEOUT);
